...
import styles from '../Styles/DanhMucStyles'; // Đảm bảo đường dẫn đúng
...
const ToppingModal: React.FC<ToppingModalProps> = ({ visible, selectedCategory, isAddingNew, topping, toppingName, priceTopping, onClose, onAddTopping }) => {
    const [newToppingName, setNewToppingName] = useState('');
    const [newToppingPrice, setNewToppingPrice] = useState('');

    const removeTopping = (toppingId: number) => {
        selectedCategory.items = selectedCategory.items.filter(topping => topping.id !== toppingId);
    };
        ...
        >
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    <Text style={styles.modalTitle}>{isAddingNew ? 'Thêm mới topping' : 'Chi tiêt topping'}</Text>
                    <TextInput
                        style={styles.input}
                    ...
                    <TouchableOpacity onPress={controllTopping} style={styles.addButton}>
                        <Text style={styles.addButtonText}>{isAddingNew ? 'Thêm' : 'Xóa'}</Text>
                    </TouchableOpacity>
