...
const ThongKeHinhThucThanhToan = () => {
  ...
  const data = [
    { name: 'Chuyển Khoản', population: revenue, color: '#ff6347' },
    { name: 'Tiền Mặt', population: promotion, color: '#90ee90' },
  ];
  ...
  const fetchRevenueData = async (type, startDate, endDate) => {
    try {
      let url = `http://${IPV4}:3000/api/thongKeHinhThucThanhToan?type=${type}`;
      ...
      if (data.length > 0) {
        setRevenue(data[0].tongChuyenKhoan || 0);
        setPromotion(data[0].tongTienMat || 0);
      } else {
        setRevenue(0);
        setPromotion(0);
      }
      console.log("Revenue (after fetch):", data[0]?.tongChuyenKhoan);
      console.log("Promotion (after fetch):", data[0]?.tongTienMat);
    } catch (error) {
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={styles.legendContainer}>
          <View style={styles.legendItemContainer}>
            <Text style={styles.legendLabel}>Chuyển Khoản ({((revenue / (revenue + promotion)) * 100).toFixed(1)}%)</Text>
            <View style={[styles.legendItem, { backgroundColor: '#ff6347' }]}>
              <Text style={styles.legendText}>{revenue.toLocaleString()} VND</Text>
            </View>
          </View>
          <View style={styles.legendItemContainer}>
            <Text style={styles.legendLabel}>Tiền Mặt ({((promotion / (revenue + promotion)) * 100).toFixed(1)}%)</Text>
...
export default ThongKeHinhThucThanhToan;
