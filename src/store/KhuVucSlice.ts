...
export const fetchKhuVucs = createAsyncThunk('khuVucs/fetchKhuVucs', async () => {
    const data = await getListKhuVuc(); // Gọi API để lấy danh sách khu vực
    return data; // Trả về dữ liệu
});

export const addNewKhuVuc = createAsyncThunk('khuVucs/addKhuVuc', async (formData: KhuVuc, thunkAPI) => {
    try {
...
const khuVucSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchKhuVucs.fulfilled, (state, action: PayloadAction<KhuVuc[]>) => {
            ...
            .addCase(addNewKhuVuc.fulfilled, (state, action: PayloadAction<KhuVuc>) => {
            ...
            .addCase(updateKhuVucThunk.fulfilled, (state, action: PayloadAction<KhuVuc>) => {
                const index = state.khuVucs.findIndex(cthd => cthd._id === action.payload._id);
                if (index !== -1) {
