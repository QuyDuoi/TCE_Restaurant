...
export const fetchBans = createAsyncThunk('bans/fetchBans', async () => {
    const data = await getListBan(); // Gọi API để lấy danh sách bàn
    return data; // Trả về dữ liệu
});
// Async thunk để thêm mới Bàn
export const addNewBan = createAsyncThunk('bans/addBans', async (formData: Ban, thunkAPI) => {
    try {
...
const banSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateBanThunk.fulfilled, (state, action: PayloadAction<Ban>) => {
                const index = state.bans.findIndex(cthd => cthd._id === action.payload._id);
                if (index !== -1) {
