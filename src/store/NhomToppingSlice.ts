...
import { getListNhomTopping, addNhomTopping, updateNhomTopping } from '../services/api';
...
export const fetchNhomTopping = createAsyncThunk('nhomTopping/fetchNhomTopping', async () => {
    const data = await getListNhomTopping();
    return data; // Trả về dữ liệu
});

// Async thunk để thêm mới NhomTopping
export const addNewNhomTopping = createAsyncThunk('nhomTopping/addNhomTopping', async (formData: NhomTopping, thunkAPI) => {
    try {
      const data = await addNhomTopping(formData);
      return data;
    } catch (error: any) {
        console.log('Lỗi thêm mới:', error);
        return thunkAPI.rejectWithValue(error.message || 'Error adding NhomTopping');
    }
  });
...
export const updateNhomToppingThunk = createAsyncThunk(
    ...
    async ({ id, formData }: { id: string, formData: NhomTopping }, thunkAPI) => {
        ...
        return thunkAPI.rejectWithValue(error.message || 'Error updating NhomTopping');
        }
    }
);
...
const nhomToppingSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchNhomTopping.fulfilled, (state, action: PayloadAction<NhomTopping[]>) => {
            ...
            .addCase(addNewNhomTopping.fulfilled, (state, action: PayloadAction<NhomTopping>) => {
            ...
            .addCase(updateNhomToppingThunk.fulfilled, (state, action: PayloadAction<NhomTopping>) => {
                const index = state.nhomToppings.findIndex(nt => nt._id === action.payload._id);
                if (index !== -1) {
            ...
            .addCase(updateNhomToppingThunk.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.payload as string || 'Error updating NhomTopping';
            });
    },
});
